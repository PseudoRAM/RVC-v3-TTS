...
class Predictor(BaseRunner):
    ...
    def run(self, text: str = Input(description='English text, 1–3000 characters. Newlines add pauses.'),
                voice: str = Input(default='VCTK226', description='English RVC target: VCTK226 (male) or VCTK231 (female). A custom model URL overrides this selection.', choices=['VCTK226', 'VCTK231']),
                ...
                seed: int = Input(default=42, ge=0, le=4294967295, description='Expressive generation random seed.'),
                custom_rvc_model_download_url: str = Input(default='', description='HTTP(S) ZIP URL containing one authorized RVC .pth checkpoint and optional .index. Overrides voice. Use trusted checkpoint sources.'),
                refresh_custom_model: bool = Input(default=False, description='Redownload the cached custom voice ZIP.')) -> Output:
        folder, _ = self.pipeline.run(Request(text, voice, source_voice, speed, pitch, pause_ms, use_index, index_rate,
            ...
            custom_rvc_model_download_url, refresh_custom_model))
